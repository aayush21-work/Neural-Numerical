...
X = torch.from_numpy((X_np - X_mean) / X_std).to(device)
Y = torch.from_numpy((Y_np - Y_mean) / Y_std).to(device)
...
for _ in range(5):
    ...
    c_t  = torch.from_numpy((c.astype(np.float32) - X_mean) / X_std).to(device)
    with torch.no_grad():
...
# ---- ML warm start: single batched GPU forward pass ----
hybrid_steps = hybrid_fails = 0
t0 = time.perf_counter()

C_norm = torch.from_numpy((all_c_np - X_mean) / X_std).to(device)
with torch.no_grad():
    x0_all = model(C_norm).cpu().numpy().flatten() * Y_std + Y_mean

# Newton polish on CPU (the actual root-finding work)
...
# ---- also time just the GPU inference separately ----
torch.cuda.synchronize()
t0 = time.perf_counter()
with torch.no_grad():
    _ = model(C_norm)
torch.cuda.synchronize()
inference_only = time.perf_counter() - t0
...
print(f"GPU inference only       : {inference_only*1000:.3f} ms  (for all {TEST_SAMPLES} samples)")
print(f"GPU inference per sample : {inference_only/TEST_SAMPLES*1e6:.3f} us")
